...
- **語りかけモード:** 定期的にカメラ撮影 + LLM 推論を繰り返す自動実行モード
  - **テキスト:** LLM に送信するテキスト (デフォルト: 「今何が見える？」)
  - **間隔:** 実行間隔を秒数で指定 (デフォルト: 60秒、最小5秒)
  - **開始/停止:** setInterval で定期実行、`/api/custom-llm` に `{ text, use_camera: true }` で POST
  - **排他制御:** `autoTalkBusy` フラグで前回の推論完了まで次をスキップ
